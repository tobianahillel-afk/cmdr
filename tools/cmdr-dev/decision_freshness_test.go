...
func TestExpiredAcceptedDecisionBecomesStale(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if fresh || !containsString(reasons, "date-expiry") { t.Fatalf("expected date expiry, got %v", reasons) }
}
...
func TestResearchOrValidationChangeInvalidatesSnapshot(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if fresh || !containsString(reasons, "research-evidence-change") { t.Fatalf("expected research drift, got %v", reasons) }
	...
	fresh, reasons, _, err = evaluateFreshnessRecord(root, mustResearchDate(t, "2026-10-01"), canonicalFreshnessPolicy(), record, decisions.Decisions[0], research, validations, state)
	if err != nil { t.Fatal(err) }
	if fresh || !containsString(reasons, "validation-evidence-change") { t.Fatalf("expected validation drift, got %v", reasons) }
}

func TestTrackedInputChangeInvalidatesSnapshot(t *testing.T) {
	root, state, _, decisions, research, validations, record := freshnessFixture(t)
	path := filepath.Join(root, "engineering", "research", "assumption.md")
	if err := os.MkdirAll(filepath.Dir(path), 0o750); err != nil { t.Fatal(err) }
	...
	fresh, reasons, _, err := evaluateFreshnessRecord(root, mustResearchDate(t, "2026-10-01"), canonicalFreshnessPolicy(), record, decisions.Decisions[0], research, validations, state)
	if err != nil { t.Fatal(err) }
	if fresh || !containsString(reasons, "repository-input-change") { t.Fatalf("expected tracked input drift, got %v", reasons) }
}
...
func TestFiredExternalSignalInvalidatesDecision(t *testing.T) {
	...
	fresh, reasons, _, err := evaluateFreshnessRecord(root, mustResearchDate(t, "2026-10-01"), canonicalFreshnessPolicy(), record, decisions.Decisions[0], research, validations, state)
	if err != nil { t.Fatal(err) }
	if fresh || !containsString(reasons, "material-new-research") { t.Fatalf("expected fired signal, got %v", reasons) }
}
