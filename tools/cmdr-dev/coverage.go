...
var openDecisionIDPattern = regexp.MustCompile(`^OPEN-[0-9]{3}package main
...
var screenIDPattern = regexp.MustCompile(`^[A-Z]{3}-[A-Z0-9]{2,4}-[0-9]{3}package main
...
var adrIDPattern = regexp.MustCompile(`^ADR-[0-9]{4}package main
...
func classifyOwnedDocument(doc SpecDocument) string {
	...
	if screenIDPattern.MatchString(doc.ID) && (strings.EqualFold(doc.Type, "screen") || strings.EqualFold(doc.SourceOfTruth, "screen") || strings.Contains(doc.Path, "/screens/")) {
		return "screen"
	}
