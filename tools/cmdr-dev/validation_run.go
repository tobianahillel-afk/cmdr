...
func runValidationExecution(root, workUnit, changesFile string, state CurrentState, graph WorkGraph) (ValidationExecutionSummary, error) {
	...
	budgetReport, err := compileEngineEfficiencyBudgetReport(root, metrics, plan)
	summary.EfficiencyBudgets = &budgetReport
	if err != nil {
		return summary, fmt.Errorf("engine efficiency budget: %w", err)
	}
	return summary, nil
}
...
func validationExecutorMode(key string) (string, error) {
	switch key {
	case "git-changes", "impact", "validation-plan", "validation-run", "metrics-snapshot", "metrics-optimizer", "metrics-budget":
...
func preflightEvidence(key string) string {
	...
	case "metrics-budget":
		return "current validation process evaluates safety floors and same-profile efficiency budgets after metric snapshot compilation"
	default:
...
func executeValidationCheck(root, tempDir, changesFile, key string, state CurrentState, graph WorkGraph) (string, error) {
	...
	case "decision-freshness-audit":
		summary, err := runDecisionFreshnessAudit(root, "", state, graph)
		if err != nil {
			return "", err
		}
	...
	case "performance-benchmark-audit":
		summary, err := runPerformanceBenchmarkAudit(root, "pr", defaultCIEnvironment, changesFile)
		if err != nil {
			return "", err
		}
	...
	case "deep-performance-audit":
		...
		return fmt.Sprintf("stage=%s runtime_boundaries=%d registered=%d selected=%d executed=%d sensitive=%t status=%s source=%s",
			summary.Stage, summary.RuntimeBoundaries, summary.RegisteredTargets, summary.SelectedTargets,
			summary.ExecutedTargets, summary.RiskSensitiveChange, summary.Status, summary.SourceSHA), nil
	...
	case "recovery-journal-audit":
		...
		return fmt.Sprintf("events=%d work_units=%d agents=%d status=%s last_sequence=%d",
			summary.Events, summary.WorkUnits, summary.Agents, summary.Status, summary.LastSeq), nil
	...
	case "pilot-contract-audit":
		...
		return fmt.Sprintf("contract=%s slice=%s boundary=%s fixtures=%d negative=%d runtime_dependencies=%d runtime_files=%d",
			summary.ContractID, summary.SliceID, summary.RuntimeBoundary, summary.Fixtures,
			summary.NegativeFixtures, summary.RuntimeDependencies, summary.RuntimeFiles), nil
	case "pilot-e2e-audit":
		summary, err := runPilotE2EAudit(root, state, graph)
		if err != nil {
			return "", err
		}
		return fmt.Sprintf("slice=%s capability=%s adversarial=%d coverage=%.2f auth=%s tenant=%s sast=%d sca=%d p95=%.9f budget=%.3f recovery=%s readiness_claim=%t limitations=%d status=%s",
			summary.SliceID, summary.Capability, summary.AdversarialTests, summary.RuntimeCoveragePercent,
	...
	case "implementation-readiness":
		...
		return fmt.Sprintf("total=%d implemented=%d ready=%d blocked=%d proposed=%d decision_blocked=%d dependency_blocked=%d unscoped_blockers=%d digest=%s status=%s",
	...
	case "event-search-contract-audit":
		summary, err := runEventSearchContractAudit(root)
		if err != nil {
			return "", err
		}
