---
id: CAP-STD-065
title: Studio Deployment, Promotion and Rollout Lifecycle
product: cmdr-studio
...
---
# CAP-STD-065 — Studio Deployment, Promotion and Rollout Lifecycle

## 1. Définition
Define Studio Deployment candidate/request semantics, exact version and target, staged rollout, promotion, partial/failure/blocked/superseded states and provenance without choosing a deployment engine or issuing real commands.

## 2. Problème utilisateur
Sans **Studio Deployment**, Studio ne peut pas distinguer de façon reproductible l’asset/version, le contexte, les preuves, les limites et le lifecycle; un score, une projection technique ou un état local pourrait alors être pris à tort pour une autorité, un succès production ou un objet d’un autre produit.
...
|---|---|---:|---|---|
| published Studio asset version | Studio | oui | exact/current | blocked/incomplete if required |
| Deployment Compatibility Assessment | Studio | oui | exact/current | blocked/incomplete if required |
| deployment target | Studio | oui | exact/current | blocked/incomplete if required |
| environment | Studio | selon scope | exact/current | blocked/incomplete if required |
| rollout scope | Studio | selon scope | exact/current | blocked/incomplete if required |
| staged rollout plan | Studio | selon scope | exact/current | blocked/incomplete if required |
| permission/authority context | Studio | selon scope | exact/current | blocked/incomplete if required |
| readiness evidence | Studio | selon scope | exact/current | blocked/incomplete if required |
| Settings runtime/config references | Studio | selon scope | exact/current | blocked/incomplete if required |
...
|---|---|---|---|
| Studio Deployment | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |
| Rollout Stage Record | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |

## 11. Fonctionnalités
published Studio asset version; Deployment Compatibility Assessment; deployment target; environment; rollout scope; staged rollout plan; permission/authority context; readiness evidence; Settings runtime/config references; provenance; validation des dépendances; version pinning; partial/inconclusive; warnings; supersession; historique.

## 12. Actions utilisateur
| Action | Rôle | Objet | Classe | Précondition | Résultat | Govern |
|---|---|---|---:|---|---|---|
| inspecter Studio Deployment | authorized user | Studio Deployment | 0 | read | contexte exact | non |
| évaluer préconditions/compatibilité | reviewer | Studio Deployment | 1 | sources lisibles | pass/warning/blocked explicable | non |
| créer/mettre à jour Studio Deployment | owner/reviewer | Studio Deployment | 2 | manage permission | record versionné | OPEN-013 |
| effectuer une transition pouvant produire un effet | authorized operator | Studio Deployment | 3 | permissions + dépendances Govern/Settings applicables | transition explicite seulement | governed |
...
`candidate`, `planned`, `deploying`, `partially-deployed`, `deployed`, `failed`, `blocked`, `superseded`, `cancelled`. Lifecycle et outcome restent distincts quand les deux existent.
...
|---|---|---|---|
| Studio Deployment | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |
| Rollout Stage Record | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |

## 17. Transitions
| Source | Déclencheur | Destination | Contexte transmis | Retour |
|---|---|---|---|---|
| sources canoniques | préconditions satisfaites | Studio Deployment | versions, tenant/env, permissions, provenance | source |
| Studio Deployment | gap/contradiction | Assurance/source owner | missing refs + limites | même lineage |
| Studio Deployment | résultat accepté | étape Studio suivante | outcome + warnings + provenance | return origin |
...
## 19. Source de vérité
**Studio Deployment** est source de sa sémantique Studio seulement. Les objets/faits externes restent chez leurs owners. Evaluation/Simulation/health/output ne deviennent pas Govern Approval/Decision/Result/Evidence automatiquement.
...
**Given** a staged rollout partially fails, **When** deployment state is reconciled, **Then** successful and failed scopes remain distinct and full rollout is not claimed.

**Given** a published version exists, **When** deployment is not requested, **Then** the version remains published but not deployed.

**Given** AI proposes deployment, **When** the proposal is reviewed, **Then** AI cannot initiate effectful deployment authority by itself.

**Given** a deployment is cross-tenant, **When** permission is absent, **Then** the request is blocked before effect and no scope expansion occurs.
