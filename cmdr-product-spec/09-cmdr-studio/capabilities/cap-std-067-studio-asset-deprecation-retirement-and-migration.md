---
id: CAP-STD-067
title: Studio Asset Deprecation, Retirement and Migration
product: cmdr-studio
...
---
# CAP-STD-067 — Studio Asset Deprecation, Retirement and Migration

## 1. Définition
Define Studio asset deprecation, replacement/supersession, disablement distinction, retirement candidates, retained history and migration planning without deleting history or automatically converting consumers.

## 2. Problème utilisateur
Sans **Deprecation Record**, Studio ne peut pas distinguer de façon reproductible l’asset/version, le contexte, les preuves, les limites et le lifecycle; un score, une projection technique ou un état local pourrait alors être pris à tort pour une autorité, un succès production ou un objet d’un autre produit.
...
| replacement/superseding version | Studio | oui | exact/current | blocked/incomplete if required |
| consumer inventory | Studio | oui | exact/current | blocked/incomplete if required |
| dependency inventory | Studio | selon scope | exact/current | blocked/incomplete if required |
| compatibility evidence | Studio | selon scope | exact/current | blocked/incomplete if required |
| deprecation notice | Studio | selon scope | exact/current | blocked/incomplete if required |
| retirement criteria | Studio | selon scope | exact/current | blocked/incomplete if required |
| migration requirement | Studio | selon scope | exact/current | blocked/incomplete if required |
| migration path | Studio | selon scope | exact/current | blocked/incomplete if required |
| retained provenance | Studio | selon scope | exact/current | blocked/incomplete if required |
...
| notification | Shared Capabilities | exact/versioned authorized projection | read/link only |
| environment | Platform Settings | exact/versioned authorized projection | read/link only |

## 10. Objets créés ou modifiés
| Objet | Opération | Propriétaire | Règle |
|---|---|---|---|
| Deprecation Record | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |
| Retirement Record | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |
| Migration Plan | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |

## 11. Fonctionnalités
asset/version; replacement/superseding version; consumer inventory; dependency inventory; compatibility evidence; deprecation notice; retirement criteria; migration requirement; migration path; retained provenance; validation des dépendances; version pinning; partial/inconclusive; warnings; supersession; historique.

## 12. Actions utilisateur
| Action | Rôle | Objet | Classe | Précondition | Résultat | Govern |
|---|---|---|---:|---|---|---|
| inspecter Deprecation Record | authorized user | Deprecation Record | 0 | read | contexte exact | non |
| évaluer préconditions/compatibilité | reviewer | Deprecation Record | 1 | sources lisibles | pass/warning/blocked explicable | non |
| créer/mettre à jour Deprecation Record | owner/reviewer | Deprecation Record | 2 | manage permission | record versionné | OPEN-013 |
| effectuer une transition pouvant produire un effet | authorized operator | Deprecation Record | 3 | permissions + dépendances Govern/Settings applicables | transition explicite seulement | governed |
...
`active`, `deprecated`, `replacement-available`, `retirement-candidate`, `retired`, `migration-required`, `migration-blocked`. Lifecycle et outcome restent distincts quand les deux existent.
...
|---|---|---|---|
| Deprecation Record | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |
| Retirement Record | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |
| Migration Plan | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |

## 17. Transitions
| Source | Déclencheur | Destination | Contexte transmis | Retour |
|---|---|---|---|---|
| sources canoniques | préconditions satisfaites | Deprecation Record | versions, tenant/env, permissions, provenance | source |
| Deprecation Record | gap/contradiction | Assurance/source owner | missing refs + limites | même lineage |
| Deprecation Record | résultat accepté | étape Studio suivante | outcome + warnings + provenance | return origin |
...
## 19. Source de vérité
**Deprecation Record** est source de sa sémantique Studio seulement. Les objets/faits externes restent chez leurs owners. Evaluation/Simulation/health/output ne deviennent pas Govern Approval/Decision/Result/Evidence automatiquement.
...
**Given** an asset is deprecated while consumers still exist, **When** deprecation is recorded, **Then** consumers are notified and the asset is not treated as retired or deleted.

**Given** retirement is requested while dependencies remain, **When** retirement eligibility is checked, **Then** retirement is blocked and dependencies remain visible.

**Given** a migration is required, **When** a Migration Plan is created, **Then** no automatic conversion or consumer mutation occurs.

**Given** an asset is retired, **When** history is inspected, **Then** prior versions, evidence and provenance remain resolvable.
