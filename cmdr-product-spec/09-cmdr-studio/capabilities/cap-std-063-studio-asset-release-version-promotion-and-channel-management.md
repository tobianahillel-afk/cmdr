---
id: CAP-STD-063
title: Studio Asset Release, Version Promotion and Channel Management
product: cmdr-studio
...
---
# CAP-STD-063 — Studio Asset Release, Version Promotion and Channel Management

## 1. Définition
Define released Studio asset versions, promotion among Studio lifecycle classifications/channels, consumer notification and deprecation/migration signals without selecting a package registry or treating promotion as deployment success.

## 2. Problème utilisateur
Sans **Studio Asset Release Record**, Studio ne peut pas distinguer de façon reproductible l’asset/version, le contexte, les preuves, les limites et le lifecycle; un score, une projection technique ou un état local pourrait alors être pris à tort pour une autorité, un succès production ou un objet d’un autre produit.
...
|---|---|---:|---|---|
| published version | Studio | oui | exact/current | blocked/incomplete if required |
| release classification/channel reference | Studio | oui | exact/current | blocked/incomplete if required |
| previous version | Studio | oui | exact/current | blocked/incomplete if required |
| compatibility evidence | Studio | selon scope | exact/current | blocked/incomplete if required |
| consumer set | Studio | selon scope | exact/current | blocked/incomplete if required |
...
| promotion request | Studio | selon scope | exact/current | blocked/incomplete if required |
| Settings administrative channel config if applicable | Studio | selon scope | exact/current | blocked/incomplete if required |
...
|---|---|---|---|
| Studio Asset Release Record | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |
| Promotion Record | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |

## 11. Fonctionnalités
published version; release classification/channel reference; previous version; compatibility evidence; consumer set; deprecation notice; migration requirement; promotion request; Settings administrative channel config if applicable; provenance; validation des dépendances; version pinning; partial/inconclusive; warnings; supersession; historique.

## 12. Actions utilisateur
| Action | Rôle | Objet | Classe | Précondition | Résultat | Govern |
|---|---|---|---:|---|---|---|
| inspecter Studio Asset Release Record | authorized user | Studio Asset Release Record | 0 | read | contexte exact | non |
| évaluer préconditions/compatibilité | reviewer | Studio Asset Release Record | 1 | sources lisibles | pass/warning/blocked explicable | non |
| créer/mettre à jour Studio Asset Release Record | owner/reviewer | Studio Asset Release Record | 2 | manage permission | record versionné | OPEN-013 |
| effectuer une transition pouvant produire un effet | authorized operator | Studio Asset Release Record | 3 | permissions + dépendances Govern/Settings applicables | transition explicite seulement | governed |
...
`published`, `release-candidate`, `promoted`, `superseded`, `deprecated-notice`. Lifecycle et outcome restent distincts quand les deux existent.
...
|---|---|---|---|
| Studio Asset Release Record | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |
| Promotion Record | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |

## 17. Transitions
| Source | Déclencheur | Destination | Contexte transmis | Retour |
|---|---|---|---|---|
| sources canoniques | préconditions satisfaites | Studio Asset Release Record | versions, tenant/env, permissions, provenance | source |
| Studio Asset Release Record | gap/contradiction | Assurance/source owner | missing refs + limites | même lineage |
| Studio Asset Release Record | résultat accepté | étape Studio suivante | outcome + warnings + provenance | return origin |
...
## 19. Source de vérité
**Studio Asset Release Record** est source de sa sémantique Studio seulement. Les objets/faits externes restent chez leurs owners. Evaluation/Simulation/health/output ne deviennent pas Govern Approval/Decision/Result/Evidence automatiquement.
...
**Given** a published version is promoted to a channel, **When** promotion completes, **Then** no deployment success or runtime enablement is inferred.

**Given** a consumer uses an incompatible previous version, **When** release compatibility is reviewed, **Then** migration/deprecation information is surfaced.

**Given** release-channel configuration is administrative, **When** Studio consumes the channel, **Then** Settings remains owner of administrative configuration.

**Given** AI is unavailable, **When** release notes are produced, **Then** human-authored summaries and deterministic version metadata remain sufficient.
