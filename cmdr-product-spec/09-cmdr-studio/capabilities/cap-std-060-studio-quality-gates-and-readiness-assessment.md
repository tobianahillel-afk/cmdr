---
id: CAP-STD-060
title: Studio Quality Gates and Readiness Assessment
product: cmdr-studio
module: assurance
...
---
# CAP-STD-060 — Studio Quality Gates and Readiness Assessment

## 1. Définition
Aggregate required evaluation, simulation, regression, permission, dependency and compatibility evidence into Studio quality-gate/readiness outcomes without equating readiness with authorization, publication or deployment.

## 2. Problème utilisateur
Sans **Quality Gate Assessment**, Studio ne peut pas distinguer de façon reproductible l’asset/version, le contexte, les preuves, les limites et le lifecycle; un score, une projection technique ou un état local pourrait alors être pris à tort pour une autorité, un succès production ou un objet d’un autre produit.
...
|---|---|---:|---|---|
| required evaluations | Studio | oui | exact/current | blocked/incomplete if required |
| required simulations | Studio | oui | exact/current | blocked/incomplete if required |
| regression status | Studio | oui | exact/current | blocked/incomplete if required |
| unresolved failures | Studio | selon scope | exact/current | blocked/incomplete if required |
| permission assurance | Studio | selon scope | exact/current | blocked/incomplete if required |
| dependency health | Studio | selon scope | exact/current | blocked/incomplete if required |
| compatibility | Studio | selon scope | exact/current | blocked/incomplete if required |
| deprecations | Studio | selon scope | exact/current | blocked/incomplete if required |
| warnings | Studio | selon scope | exact/current | blocked/incomplete if required |
| human review | Studio | selon scope | exact/current | blocked/incomplete if required |
...
|---|---|---|---|
| Quality Gate Assessment | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |
| Readiness Assessment | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |

## 11. Fonctionnalités
required evaluations; required simulations; regression status; unresolved failures; permission assurance; dependency health; compatibility; deprecations; warnings; human review; validation des dépendances; version pinning; partial/inconclusive; warnings; supersession; historique.

## 12. Actions utilisateur
| Action | Rôle | Objet | Classe | Précondition | Résultat | Govern |
|---|---|---|---:|---|---|---|
| inspecter Quality Gate Assessment | authorized user | Quality Gate Assessment | 0 | read | contexte exact | non |
| évaluer préconditions/compatibilité | reviewer | Quality Gate Assessment | 1 | sources lisibles | pass/warning/blocked explicable | non |
| créer/mettre à jour Quality Gate Assessment | owner/reviewer | Quality Gate Assessment | 2 | manage permission | record versionné | OPEN-013 |
| effectuer une transition pouvant produire un effet | authorized operator | Quality Gate Assessment | 3 | permissions + dépendances Govern/Settings applicables | transition explicite seulement | governed |
...
`ready-for-review`, `review-required`, `blocked`, `incomplete`, `warning`, `not-ready`, `superseded`. Lifecycle et outcome restent distincts quand les deux existent.
...
|---|---|---|---|
| Quality Gate Assessment | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |
| Readiness Assessment | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |

## 17. Transitions
| Source | Déclencheur | Destination | Contexte transmis | Retour |
|---|---|---|---|---|
| sources canoniques | préconditions satisfaites | Quality Gate Assessment | versions, tenant/env, permissions, provenance | source |
| Quality Gate Assessment | gap/contradiction | Assurance/source owner | missing refs + limites | même lineage |
| Quality Gate Assessment | résultat accepté | étape Studio suivante | outcome + warnings + provenance | return origin |
...
## 19. Source de vérité
**Quality Gate Assessment** est source de sa sémantique Studio seulement. Les objets/faits externes restent chez leurs owners. Evaluation/Simulation/health/output ne deviennent pas Govern Approval/Decision/Result/Evidence automatiquement.
...
**Given** a required quality gate is blocked, **When** readiness is calculated, **Then** the asset is not marked ready-for-review.

**Given** only a non-blocking warning remains, **When** readiness is calculated, **Then** the warning is preserved and the outcome follows the declared criterion rather than silent promotion.

**Given** all Studio gates pass, **When** readiness is ready-for-review, **Then** no publication, Govern Approval or deployment is created automatically.

**Given** AI is unavailable, **When** readiness is evaluated, **Then** deterministic gate aggregation and human review remain sufficient.
