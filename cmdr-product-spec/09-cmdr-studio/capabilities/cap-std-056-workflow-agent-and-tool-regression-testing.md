---
id: CAP-STD-056
title: Workflow, Agent and Tool Regression Testing
product: cmdr-studio
module: assurance
...
---
# CAP-STD-056 — Workflow, Agent and Tool Regression Testing

## 1. Définition
Compare baseline and candidate Studio asset versions across prior and current evaluations to detect behavioral, compatibility, reliability or dependency regressions without equating regression with bug or root cause.

## 2. Problème utilisateur
Sans **Regression Assessment**, Studio ne peut pas distinguer de façon reproductible l’asset/version, le contexte, les preuves, les limites et le lifecycle; un score, une projection technique ou un état local pourrait alors être pris à tort pour une autorité, un succès production ou un objet d’un autre produit.
...
|---|---|---:|---|---|
| baseline version | Studio | oui | exact/current | blocked/incomplete if required |
| candidate version | Studio | oui | exact/current | blocked/incomplete if required |
| prior Evaluation Results | Studio | oui | exact/current | blocked/incomplete if required |
| current Evaluation Results | Studio | selon scope | exact/current | blocked/incomplete if required |
| changed Tool/Skill/Workflow/Agent references | Studio | selon scope | exact/current | blocked/incomplete if required |
| provider/runtime reference changes | canonical owner | selon scope | exact/current | blocked/incomplete if required |
| compatibility observations | Studio | selon scope | exact/current | blocked/incomplete if required |
| performance/reliability observations | Studio | selon scope | exact/current | blocked/incomplete if required |
...
|---|---|---|---|
| Regression Assessment | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |
| Regression Difference | create/review/supersede as applicable | CMDR Studio | no implicit source-owner mutation |

## 11. Fonctionnalités
baseline version; candidate version; prior Evaluation Results; current Evaluation Results; changed Tool/Skill/Workflow/Agent references; provider/runtime reference changes; compatibility observations; performance/reliability observations; validation des dépendances; version pinning; partial/inconclusive; warnings; supersession; historique.

## 12. Actions utilisateur
| Action | Rôle | Objet | Classe | Précondition | Résultat | Govern |
|---|---|---|---:|---|---|---|
| inspecter Regression Assessment | authorized user | Regression Assessment | 0 | read | contexte exact | non |
| évaluer préconditions/compatibilité | reviewer | Regression Assessment | 1 | sources lisibles | pass/warning/blocked explicable | non |
| créer/mettre à jour Regression Assessment | owner/reviewer | Regression Assessment | 2 | manage permission | record versionné | OPEN-013 |
| effectuer une transition pouvant produire un effet | authorized operator | Regression Assessment | 3 | permissions + dépendances Govern/Settings applicables | transition explicite seulement | governed |
...
`no-regression`, `regression-candidate`, `improvement`, `inconclusive`, `blocked`. Lifecycle et outcome restent distincts quand les deux existent.
...
|---|---|---|---|
| Regression Assessment | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |
| Regression Difference | Studio record | downstream Studio assurance/lifecycle | version/scope/limits/provenance explicit |

## 17. Transitions
| Source | Déclencheur | Destination | Contexte transmis | Retour |
|---|---|---|---|---|
| sources canoniques | préconditions satisfaites | Regression Assessment | versions, tenant/env, permissions, provenance | source |
| Regression Assessment | gap/contradiction | Assurance/source owner | missing refs + limites | même lineage |
| Regression Assessment | résultat accepté | étape Studio suivante | outcome + warnings + provenance | return origin |
...
## 19. Source de vérité
**Regression Assessment** est source de sa sémantique Studio seulement. Les objets/faits externes restent chez leurs owners. Evaluation/Simulation/health/output ne deviennent pas Govern Approval/Decision/Result/Evidence automatiquement.
...
**Given** a Workflow candidate newly fails a previously passing case, **When** regression comparison runs, **Then** a regression candidate is recorded with exact baseline and candidate versions.

**Given** a Tool version changes after baseline, **When** behavior differs, **Then** the difference is linked to changed dependencies when supported but root cause is not invented.

**Given** a newly passing case appears, **When** results are compared, **Then** the improvement is recorded separately from regressions.

**Given** AI is unavailable, **When** regression testing runs, **Then** deterministic result comparison remains functional.
