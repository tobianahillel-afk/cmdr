...
## 1. Définition
Inspecter en lecture les processus, sessions, services, connexions et contexte système, puis préparer une collecte ou une action risquée.

## 2. Problème utilisateur
Sans Process and System Inspection, l’utilisateur perd le lien entre le Case, l’Endpoint, l’autorité applicable, l’exécution locale et les résultats. Les états partiels ou offline peuvent alors être pris pour un succès et les objets peuvent être confondus.

## 3. Objectifs
- fournir Endpoint snapshot, catégories système, période, policy;
...
- produire snapshot d’inspection, Artifact optionnel, action draft sans transférer l’ownership.
...
| Données spécifiques | Endpoint snapshot, catégories système, période, policy | données locales | selon opération | source/version visibles | résultat partiel explicite |
...
|---|---|---|---|
| Record métier Process and System Inspection | créer, mettre à jour ou supersede conceptuellement | Investigate, modèle final à Phase Objets | versionné, Case-scoped, sans machine finale |
...
## 11. Fonctionnalités
- filtrer; inspecter; demander collecte; préparer action;
...
`available`, `loading-snapshot`, `partial`, `stale`, `restricted`, `unsupported`, `failed`. Ces dimensions sont Draft et ne finalisent aucune machine d’état objet.
...
|---|---|---|---|
| Snapshot d’inspection, Artifact optionnel, action draft | record, relation ou événement métier | Case Workspace et capabilities dépendantes | cible, scope, acteur, statut, erreurs et version visibles |
...
**Given** un Case, un Endpoint disponible et un utilisateur autorisé **When** il utilise Process and System Inspection **Then** cible, scope, policy, classe, progression, résultat et retour au Case sont visibles sans transfert d’ownership.
