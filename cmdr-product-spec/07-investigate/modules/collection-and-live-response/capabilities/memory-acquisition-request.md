...
## 1. Définition
Préparer, demander et suivre une acquisition mémoire fonctionnelle sans choisir de moteur, format ou plateforme supportée.

## 2. Problème utilisateur
Sans Memory Acquisition Request, l’utilisateur perd le lien entre le Case, l’Endpoint, l’autorité applicable, l’exécution locale et les résultats. Les états partiels ou offline peuvent alors être pris pour un succès et les objets peuvent être confondus.

## 3. Objectifs
- fournir Case, Endpoint, support déclaré, impact, policy, permission;
...
- produire Memory Image/Artifact ou erreur explicite sans transférer l’ownership.
...
## 6. Utilisateurs
Principal : DFIR Analyst. Secondaires : Investigation Lead, Evidence Reviewer, Incident Commander, approbateur Govern ou Platform Administrator en consultation selon la capability.
...
| Données spécifiques | Case, Endpoint, support déclaré, impact, policy, permission | données locales | selon opération | source/version visibles | résultat partiel explicite |
...
|---|---|---|---|
| Record métier Memory Acquisition Request | créer, mettre à jour ou supersede conceptuellement | Investigate, modèle final à Phase Objets | versionné, Case-scoped, sans machine finale |
...
## 11. Fonctionnalités
- préparer; demander; suivre; annuler;
...
`draft`, `unsupported`, `policy-blocked`, `awaiting-approval`, `queued`, `acquiring`, `partial`, `completed`, `failed`, `cancelled`. Ces dimensions sont Draft et ne finalisent aucune machine d’état objet.
...
|---|---|---|---|
| Memory Image/Artifact ou erreur explicite | record, relation ou événement métier | Case Workspace et capabilities dépendantes | cible, scope, acteur, statut, erreurs et version visibles |
...
**Given** un Case, un Endpoint disponible et un utilisateur autorisé **When** il utilise Memory Acquisition Request **Then** cible, scope, policy, classe, progression, résultat et retour au Case sont visibles sans transfert d’ownership.
