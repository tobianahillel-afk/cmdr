...
The register is split into active shards. IDs are immutable and never recycled. The global file is the authoritative index and total; each shard carries row-level owner, role, object, delivery and traceability evidence. Documentary PASS never proves implementation.
...
- Govern: **47** capabilities, 1269 sections, 282 mandatory tables; GOV-1/2/3 historical gates 180/190/200 remain PASS.
...
- proposed: **2**;
- planned: **333**;
- implemented/native/integrated claims introduced by STD-1: **0**.

## Studio state
`CAP-STD-001..016` are unique, newly allocated, non-recycled, `draft / defined / planned`. STD-1 is an execution lot under Delivery Roadmap Phase 5; it is not Phase 5A/5B and does not start STD-2/STD-3/STD-4 or Endpoint.

No capability is presented as runtime-available solely because a documentary specification exists.
