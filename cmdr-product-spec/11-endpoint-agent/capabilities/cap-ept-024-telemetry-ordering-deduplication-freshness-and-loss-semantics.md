---
id: CAP-EPT-024
title: Telemetry Ordering, Deduplication, Freshness and Loss Semantics
...
---
# CAP-EPT-024 — Telemetry Ordering, Deduplication, Freshness and Loss Semantics

## 1. Définition
Define event identity/correlation, ordering, late arrival, duplicate candidates, freshness, sequence-gap and loss-candidate semantics without asserting causality or tampering.

## 2. Problème utilisateur
Consumers need honest telemetry quality semantics in the presence of reordering, duplicates, lateness and gaps.

## 3. Objectifs
- Represent event/observation identity references, sequence/correlation, duplicate candidate, late/reordered/stale states, expected cadence where meaningful, gap/loss candidate and recovery/resumption.
- Distinguish telemetry gaps from sensor failure, tampering and endpoint offline state.

## 4. Non-objectifs
- No exactly-once guarantee; no causal-ordering guarantee; no tamper verdict; no storage/buffer implementation.
...
|---|---|---|---|
| Telemetry Event Projection | Endpoint Agent | source-owned projection only | read/reference within permission |
| Observation Source | Endpoint Agent | source-owned projection only | read/reference within permission |
| Heartbeat | Endpoint Agent | source-owned projection only | read/reference within permission |

## 10. Objets créés ou modifiés
| Objet | Opération | Propriétaire | Règle |
|---|---|---|---|
| Duplicate Observation Candidate | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Freshness State | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Telemetry Gap | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Loss Candidate | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Ordering State | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`ordered`, `out-of-order`, `late`, `duplicate-candidate`, `stale`, `gap-candidate`, `loss-suspected`, `resumed`, `unknown`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| quality-state projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| gap/loss candidate | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| freshness/ordering context | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
**Given** an event arrives after newer events, **When** ordering is assessed, **Then** it is marked late/out-of-order without changing event time or asserting causality.

**Given** two events share duplicate indicators, **When** deduplication is assessed, **Then** they become duplicate candidates rather than proof of duplicate activity.

**Given** an expected sequence has a gap, **When** quality is assessed, **Then** a gap/loss candidate is recorded without automatically asserting tampering or sensor failure.
