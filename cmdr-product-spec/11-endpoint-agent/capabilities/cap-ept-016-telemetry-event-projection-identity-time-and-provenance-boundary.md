---
id: CAP-EPT-016
title: Telemetry Event Projection, Identity, Time and Provenance Boundary
...
---
# CAP-EPT-016 — Telemetry Event Projection, Identity, Time and Provenance Boundary

## 1. Définition
Define how Endpoint local observations project into the Shared-owned immutable `telemetry-event` envelope with identity, time and provenance semantics while preserving source facts.

## 2. Problème utilisateur
Consumers need consistent event identity/time/provenance without transferring ownership of the generic event envelope from Shared to Endpoint.

## 3. Objectifs
- Map Endpoint observation facts to Shared telemetry-event references, distinguish source/observation/ingestion timestamps, preserve raw/source references and normalization state.
- Expose completeness/limitations without defining a physical event schema.

## 4. Non-objectifs
- No new EndpointTelemetryEvent canonical object; no physical schema; no transport; no ingestion engine; no causal-ordering guarantee.
...
| source/capability facts | Shared telemetry-event | functional facts | oui | source-owned | unknown/degraded rather than inferred |
...
|---|---|---|---|
| Telemetry Event Projection | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Observation Time Context | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Endpoint Provenance Reference | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`source-observed`, `projected`, `normalized`, `partially-normalized`, `unmapped`, `late`, `unknown`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| Shared telemetry-event projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| time-semantics projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| source-to-event provenance chain | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
**Given** an observation has source time and a later receipt time, **When** it is projected, **Then** event/source time and ingestion/receipt time remain distinct.

**Given** a source field is not normalizable, **When** projection occurs, **Then** the raw/source reference and unmapped limitation are preserved.

**Given** an event is delivered to Investigate, **When** it is consumed, **Then** the event does not automatically become Evidence or a Finding.
