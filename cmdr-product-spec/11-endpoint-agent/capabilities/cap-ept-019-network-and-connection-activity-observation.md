---
id: CAP-EPT-019
title: Network and Connection Activity Observation
...
---
# CAP-EPT-019 — Network and Connection Activity Observation

## 1. Définition
Define network, connection, listener, flow and DNS-related endpoint observations with local/remote references, direction, process attribution, time and source limitations.

## 2. Problème utilisateur
Consumers need network activity facts without turning connection telemetry into packet capture, network detection or a malicious-connection verdict.

## 3. Objectifs
- Represent connection/listener/flow facts, local/remote endpoint references, conceptual protocol family, direction, process relation and DNS relationship where available.
- Preserve rate/privacy/source limitations and source-dependent attribution.

## 4. Non-objectifs
- No packet capture; no protocol implementation; no network Detection; no malicious-connection verdict; no active scanning.
...
| source/capability facts | network-telemetry | functional facts | oui | source-owned | unknown/degraded rather than inferred |
...
|---|---|---|---|
| Network Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Connection Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| DNS Observation Reference | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`observed`, `partial`, `rate-limited`, `sampled`, `unavailable`, `unsupported`, `unknown`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| network/connection observation | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| DNS relationship projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| process attribution when available | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
**Given** a connection is observed with process attribution, **When** it is projected, **Then** the connection and process reference are retained without a malicious verdict.

**Given** DNS telemetry is encrypted or cache-limited, **When** coverage is assessed, **Then** the limitation is explicit rather than inferred as source failure.

**Given** rate controls reduce network events, **When** consumers inspect coverage, **Then** sampling/rate state is exposed and is not misreported as unexpected loss.
