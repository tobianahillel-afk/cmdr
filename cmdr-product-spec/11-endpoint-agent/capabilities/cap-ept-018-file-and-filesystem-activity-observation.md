---
id: CAP-EPT-018
title: File and Filesystem Activity Observation
...
---
# CAP-EPT-018 — File and Filesystem Activity Observation

## 1. Définition
Define file and filesystem activity observations for create, modify, rename, delete and metadata changes with path/process relations, policy-bounded hashes and provenance.

## 2. Problème utilisateur
Consumers need file-change facts while keeping continuous telemetry distinct from forensic file acquisition and respecting sensitive paths.

## 3. Objectifs
- Represent file operation, path reference, process relation, timestamps, source, policy-bounded hash/reference and limitations.
- Apply sensitive-path minimization and explicit unsupported/unknown states.

## 4. Non-objectifs
- No file content acquisition; no quarantine; no malicious-file verdict; no physical filesystem watcher design.
...
| source/capability facts | file-telemetry | functional facts | oui | source-owned | unknown/degraded rather than inferred |
...
|---|---|---|---|
| File Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Filesystem Metadata Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Sensitive Path Projection | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`observed`, `partial`, `restricted`, `sampled`, `unavailable`, `unsupported`, `unknown`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| file/filesystem observation | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| path/process relationship projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| hash reference when policy permits | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
**Given** a file rename is observed, **When** it is projected, **Then** the old/new path references and source time are preserved when available.

**Given** a path is sensitive, **When** telemetry is projected, **Then** minimized or masked path semantics are used according to policy.

**Given** a file event exists, **When** an analyst requests file acquisition, **Then** the observation does not itself create a Collection request or Evidence.
