---
id: CAP-EPT-023
title: Telemetry Normalization, Classification and Common Field Semantics
...
---
# CAP-EPT-023 — Telemetry Normalization, Classification and Common Field Semantics

## 1. Définition
Define Endpoint-specific normalization/classification semantics and mapping into the Shared normalization boundary while preserving raw/source fields, unknown values and fidelity loss.

## 2. Problème utilisateur
Consumers need coherent fields across Endpoint sources without pretending source-specific fields are universal or selecting a final external schema.

## 3. Objectifs
- Define raw-to-normalized projection, event families, common/source-specific fields, unknown/unmapped values, version-aware mappings, fidelity-loss indicators and raw-reference provenance.
- Preserve Shared ownership of generic telemetry normalization and event envelope.

## 4. Non-objectifs
- No final ECS/OCSF/Sigma or other schema selection; no parser implementation; no physical schema; no destructive raw rewrite.
...
| source/capability facts | Shared telemetry-normalization | functional facts | oui | source-owned | unknown/degraded rather than inferred |
...
|---|---|---|---|
| Local Observation | Endpoint Agent | source-owned projection only | read/reference within permission |
| telemetry-event | Shared Capabilities | source-owned projection only | read/reference within permission |
| Parser Reference | Canonical source owner | source-owned projection only | read/reference within permission |

## 10. Objets créés ou modifiés
| Objet | Opération | Propriétaire | Règle |
|---|---|---|---|
| Normalized Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Observation Classification | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Normalization State | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Fidelity Limitation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`raw`, `partially-normalized`, `normalized`, `unmapped`, `mapping-unknown`, `fidelity-reduced`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| normalized projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| classification/family projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| unmapped/fidelity limitation | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
- telemetry-event
- source-specific Telemetry docs
- OPEN-008
...
**Given** a source field has no common mapping, **When** normalization runs, **Then** the source field remains source-specific/unmapped and is not forced into an incorrect common field.

**Given** normalization loses source precision, **When** projection is produced, **Then** fidelity reduction is explicit and raw/source reference is retained.

**Given** a normalization standard is not canonically selected, **When** mapping is documented, **Then** no external standard is chosen arbitrarily.
