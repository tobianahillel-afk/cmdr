---
id: CAP-EPT-022
title: Sensor, Component and Security-State Observation
...
---
# CAP-EPT-022 — Sensor, Component and Security-State Observation

## 1. Définition
Define observations about local sensor/component identity, enabled/disabled/degraded state, health and bounded security-state facts, with explicit capability impact.

## 2. Problème utilisateur
Operators need to distinguish sensor health and security-state facts from telemetry completeness and from security verdicts.

## 3. Objectifs
- Represent sensor/component identity, health, enabled/disabled/degraded state, source availability, capability impact, timestamp and provenance.
- Keep sensor healthy distinct from complete telemetry and security state distinct from incident/detection.

## 4. Non-objectifs
- No anti-tamper design; no security verdict; no incident creation; no component update/repair implementation.
...
| source/capability facts | CAP-EPT-008/011/015 | functional facts | oui | source-owned | unknown/degraded rather than inferred |
...
| Agent Health | Endpoint Agent | source-owned projection only | read/reference within permission |
| Technical Capability Advertisement | Endpoint Agent | source-owned projection only | read/reference within permission |

## 10. Objets créés ou modifiés
| Objet | Opération | Propriétaire | Règle |
|---|---|---|---|
| Sensor Health Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Component State Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Security-State Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Capability Impact Projection | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`healthy`, `degraded`, `disabled`, `unavailable`, `unknown`, `stale`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| sensor/component observation | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| capability-impact projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| security-state fact | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
## 18. Dépendances
- CAP-EPT-008/011/015
- security local audit/privacy boundaries
- OPEN-008
...
**Given** a sensor reports healthy, **When** no events arrive in an expected window, **Then** sensor health remains distinct from telemetry completeness and a gap candidate is recorded.

**Given** a component is disabled by an administrative setting, **When** availability is recalculated, **Then** affected observation capabilities become degraded/unavailable with source reason.

**Given** a security-state fact changes, **When** it is emitted, **Then** the fact does not automatically create an Incident or Detection.
