---
id: CAP-EPT-026
title: Sensitive Telemetry, Privacy and Masking Boundaries
...
---
# CAP-EPT-026 — Sensitive Telemetry, Privacy and Masking Boundaries

## 1. Définition
Define sensitive telemetry classification, minimization, masking/redaction, tenant isolation and projection/export restrictions for Endpoint observations.

## 2. Problème utilisateur
Endpoint telemetry may contain paths, command lines, user/session data and other sensitive metadata that must remain useful without exposing secrets or violating scope.

## 3. Objectifs
- Classify sensitive fields, require masking/minimization, preserve tenant isolation, restrict user/session/process metadata and export preparation, and retain provenance of transformations.
- Apply source/policy restrictions without hiding the existence of gaps or source limitations.

## 4. Non-objectifs
- No raw secret capture; no final retention policy; no final RBAC/ABAC; no unrestricted cross-tenant read; no export engine implementation.
...
| source/capability facts | security/privacy.md | functional facts | oui | source-owned | unknown/degraded rather than inferred |
...
|---|---|---|---|
| Sensitive Telemetry Projection | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Masking State | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Restriction Context | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`clear`, `restricted`, `masked`, `redacted`, `minimized`, `denied`, `unknown`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| policy-compliant projection | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| masking/redaction provenance | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| restriction/denial context | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
## 18. Dépendances
- security/privacy.md
- Shared secure export
- Settings Policy/tenant
- OPEN-008
...
**Given** telemetry contains a sensitive path, **When** projection is prepared, **Then** the path is minimized/masked according to policy and transformation provenance is retained.

**Given** a cross-tenant read is attempted, **When** authorization is evaluated, **Then** access is denied and no telemetry crosses tenant scope.

**Given** a command line may expose a secret, **When** telemetry is handled, **Then** raw secret capture is prohibited and restricted fields are redacted or omitted.
