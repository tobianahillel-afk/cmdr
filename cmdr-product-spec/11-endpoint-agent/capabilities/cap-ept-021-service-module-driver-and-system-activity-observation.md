---
id: CAP-EPT-021
title: Service, Module, Driver and System Activity Observation
...
---
# CAP-EPT-021 — Service, Module, Driver and System Activity Observation

## 1. Définition
Define system-oriented observations for boot/shutdown, service state, modules/images, drivers/kernel components, scheduled tasks, registry facts and source-dependent workload runtime context.

## 2. Problème utilisateur
Consumers need low-level system facts without assuming universal platform coverage or turning them into detection or investigation conclusions.

## 3. Objectifs
- Represent boot/shutdown, service/module/driver/task/system state facts, registry key/value activity where applicable, health context and source/platform limitations.
- Allow cloud-workload/container runtime identity facts only when declared by the source, without claiming platform delivery.

## 4. Non-objectifs
- No kernel/driver implementation; no registry remediation; no persistence verdict; no cloud/container support promise; no Endpoint Investigation workflow.
...
| source/capability facts | system-telemetry | functional facts | oui | source-owned | unknown/degraded rather than inferred |
...
|---|---|---|---|
| System Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Service Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Module Driver Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Registry Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
| Workload Context Observation | derive/create/update documentary functional state | Endpoint Agent | no foreign ownership transfer; source/provenance explicit |
...
`observed`, `partial`, `platform-dependent`, `source-dependent`, `unavailable`, `unsupported`, `unknown`. State names are functional/documentary semantics; they do not define a physical state machine or runtime implementation.
...
|---|---|---|---|
| system/service/module observation | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| registry/workload context projection when declared | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
| source limitation state | Endpoint technical fact/projection | authorized Endpoint/Shared/Investigate/Settings/Studio/Govern/Command consumers as applicable | source, scope, freshness, limitations and provenance explicit |
...
**Given** a driver load fact is observed, **When** it is projected, **Then** it remains a system fact and not a rootkit or malicious verdict.

**Given** registry telemetry is declared Windows-only, **When** a non-Windows endpoint is assessed, **Then** the observation class is unsupported/platform-dependent, not globally available.

**Given** cloud workload telemetry is optional, **When** the required source is absent, **Then** workload observation is unavailable/unknown without implying endpoint failure.
