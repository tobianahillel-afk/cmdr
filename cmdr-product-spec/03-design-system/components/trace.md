...
Exposer provenance et chaîne d’exécution. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
source, transformations, rules/tools, actor/run, versions, correlation IDs.

## 4. Variants

object provenance, automation, decision.
...
Aucune trace cachée derrière ia; redaction permission-aware. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

tree/list keyboard; expand/collapse; copy IDs.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.trace.*`. Aucun hex de marque direct.
