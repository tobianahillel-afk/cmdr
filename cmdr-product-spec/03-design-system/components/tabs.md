...
Changer de section ou d’artefact dans un même workspace. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
tablist, tab, active indicator, dirty/pinned marker, overflow.

## 4. Variants

section, artifact, technical overflow.
...
Six tabs techniques visibles max; route only if durable. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

arrows tabs; Home/End; Enter/Space activate; Delete closes when allowed.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.tabs.*`. Aucun hex de marque direct.
