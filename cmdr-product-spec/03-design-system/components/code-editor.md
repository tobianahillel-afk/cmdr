...
Lire ou éditer code, query et configuration. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
toolbar, file/query identity, editor, diagnostics, status, outline.

## 4. Variants

read-only, editable, diff-linked.
...
Pas de ligatures imposées; secrets redacted; family alias open-004. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

standard editor shortcuts; escape to region; accessible text mode.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.code-editor.*`. Aucun hex de marque direct.
