...
Naviguer et lancer des commandes autorisées. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
input, groups, permission/risk metadata, shortcut, result.

## 4. Variants

navigation, command, contextual, automation entry.
...
Pas chatbot; actions risquées ouvrent govern/confirmation. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Ctrl/Cmd+K; arrows; Enter; Esc; screen-reader result count.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.command-palette.*`. Aucun hex de marque direct.
