...
Montrer le contexte actif et hérité. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
tenant, environment, context items, source markers, remove/back, overflow.

## 4. Variants

global, object-chain, cross-product.
...
Sous header; priorité tenant→env→active→parent; pas barre d’actions. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Tab items; arrows optional roving; Delete removes allowed context.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.context-bar.*`. Aucun hex de marque direct.
