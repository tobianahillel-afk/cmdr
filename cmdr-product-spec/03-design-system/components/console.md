...
Afficher logs, output, debugger ou terminal autorisé. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
context/target, tabs, stream, command line optional, status, clear/copy.

## 4. Variants

logs, output, terminal, debug.
...
Une seule console; pas empilée; commandes auditées. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Ctrl+` toggle; search; pause stream; accessible transcript.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.console.*`. Aucun hex de marque direct.
