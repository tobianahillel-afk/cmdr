...
Scanner, comparer et agir sur des objets denses. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
caption, header, rows, selection, sort, resize, pagination/status.

## 4. Variants

static table, interactive grid, virtualized.
...
Html table when possible; cards forbidden as replacement. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

arrows or Tab model declared; Space select; Enter open; Home/End.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.data-table.*`. Aucun hex de marque direct.
