...
Inspecter et agir sur la sélection. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
header, Overview, Why, Relations, Evidence, Actions, Activity, Trace, resize, close.

## 4. Variants

default, pinned, responsive-fullscreen.
...
Un seul inspector; 320–560 px; six états; object title always visible. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

] opens; section navigation; Esc closes and restores focus.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.inspector.*`. Aucun hex de marque direct.
