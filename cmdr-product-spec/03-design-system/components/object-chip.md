...
Identifier et lier un objet canonique. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
type, label, identifier, source product, optional status/remove.

## 4. Variants

inline, compact, removable context.
...
Ne redéfinit jamais l’objet; lien permission-aware. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Tab if link; Enter open; Delete removes only when allowed.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.object-chip.*`. Aucun hex de marque direct.
