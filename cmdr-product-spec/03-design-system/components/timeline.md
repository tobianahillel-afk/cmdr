...
Ordonner événements et actions. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
axis, timezone, event markers, type, source, uncertainty, list alternative.

## 4. Variants

linear, grouped, zoomable.
...
Observed/inferred/decision/action distincts; alternative list. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

arrows events; +/- zoom; Enter inspect.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.timeline.*`. Aucun hex de marque direct.
