...
Montrer la hiérarchie de navigation. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
ordered links, current page, overflow.
...
Ne duplique pas le context bar ni les objets hérités. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Tab links; current not link.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.breadcrumbs.*`. Aucun hex de marque direct.
