...
Présenter une approval sans la confondre avec decision. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
request, approver, authority, SoD, result, conditions, justification, expiration.

## 4. Variants

request, pending, approved, rejected, expired, conditional.
...
Approval ≠ decision; no isolated red approve button. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

actions labeled; step-up; focus to validation errors.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.approval-card.*`. Aucun hex de marque direct.
