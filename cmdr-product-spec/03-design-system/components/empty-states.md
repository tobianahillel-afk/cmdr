...
Expliquer absence, chargement ou dégradation. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
title, reason, preserved data, safe action, correlation/freshness when relevant.

## 4. Variants

empty, loading, partial, error, offline, permission-denied.
...
Aucune illustration décorative ou donnée fictive; permission sans fuite. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

focus on heading for blocking state; retry explicit.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.empty-states.*`. Aucun hex de marque direct.
