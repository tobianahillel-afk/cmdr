...
Rechercher dans un périmètre explicite. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
scope, input, syntax/help, history, result list.

## 4. Variants

global, local, object, query-assisted.
...
Tenant, type, fraîcheur et permission visibles; ia facultative. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Ctrl/Cmd+K global; / local; arrows results; Enter open.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.search.*`. Aucun hex de marque direct.
