...
Déclencher une action explicite. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
label, optional icon, progress, shortcut.

## 4. Variants

primary, secondary, tertiary, destructive, icon, split exceptionnel.
...
Une seule primary par région; destructive jamais couleur seule. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Tab; Enter/Space active; destructive opens governed confirmation.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.buttons.*`. Aucun hex de marque direct.
