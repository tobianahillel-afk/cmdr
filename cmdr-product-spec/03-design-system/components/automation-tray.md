...
Montrer automatisations contextuelles sans remplacer le workspace. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
source context, agent/workflow, run, Tool Calls, steps, output, pause/stop/resume, trace.

## 4. Variants

closed, suggestions, runs, run-detail.
...
Fermé par défaut; manuel/déterministe visible; pas chat. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

shortcut opens; list navigation; Esc close; controls explicit.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.automation-tray.*`. Aucun hex de marque direct.
