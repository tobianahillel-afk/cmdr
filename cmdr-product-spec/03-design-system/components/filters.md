...
Réduire temporairement un jeu de données. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
scope, field, operator, value, applied count, clear/reset.

## 4. Variants

simple, advanced builder, chips.
...
Applied filters visible and url-safe; invalid state explained. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

Tab controls; Enter apply; Delete removes focused chip.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.filters.*`. Aucun hex de marque direct.
