...
Naviguer une hiérarchie ou collection technique. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
tree label, nodes, expansion, current path, search.

## 4. Variants

navigation, artifact explorer, outline.
...
Aria-tree seulement si comportement réel; liste alternative. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

arrows navigate; Right expand; Left collapse/parent; Home/End.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.tree.*`. Aucun hex de marque direct.
