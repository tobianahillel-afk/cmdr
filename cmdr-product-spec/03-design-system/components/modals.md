...
Bloquer pour décision courte ou confirmation. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
title, consequence, body, primary/secondary, close rules.

## 4. Variants

confirmation, step-up, choice, critical.
...
Pas de table complexe, workflow long ou modal imbriquée. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

focus trap; Esc only when safe; restore trigger.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.modals.*`. Aucun hex de marque direct.
