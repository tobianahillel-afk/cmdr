...
Afficher un libellé compact non suffisant seul. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
text, optional icon, accessible expansion.

## 4. Variants

category, severity, confidence, count.
...
Badge ne remplace pas phrase, statut ou permission. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

not focusable unless interactive; tooltip not sole content.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.badges.*`. Aucun hex de marque direct.
