...
Identifier une personne, équipe ou système. Le composant fournit un contrat partagé ; le produit propriétaire fournit objets, actions et données.
...
image/initials, name, presence optional.
...
Pas d’avatar humanoïde pour ia; nom et type toujours accessibles. Les mises à jour préservent focus, sélection et données valides. Toute action à effet indique portée et résultat.

## 8. Keyboard

focus only if actionable.
...
`color.surface.*`, `color.text.*`, `color.border.*`, `color.focus.ring`, `space.*`, `radius.*`, `motion.*` et `component.avatars.*`. Aucun hex de marque direct.
