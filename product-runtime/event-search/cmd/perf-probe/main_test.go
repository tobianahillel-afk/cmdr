...
func TestRunCLILatencyJSON(t *testing.T) {
	...
	if got.ElapsedNS <= 0 || got.NSPerOperation <= 0 ||
		math.IsNaN(got.NSPerOperation) || math.IsInf(got.NSPerOperation, 0) {
...
func TestRunProbeResource(t *testing.T) {
	...
	if got.Mode != "resource" || got.Operations != 1000 ||
		got.ElapsedNS <= 0 || got.NSPerOperation <= 0 || got.PeakHeapBytes == 0 {
...
func TestRunCLIRejectsInvalidArguments(t *testing.T) {
	for _, args := range [][]string{{"-unknown"}, {"unexpected"}} {
		if err := runCLI(args, &bytes.Buffer{}); err == nil {
			t.Fatalf("expected CLI argument rejection for %v", args)
		}
	}
}
